...
KEYSTORE = os.path.join(get_base_dir(), "libs", "key.jks")
...
def task(task_name, fun, *args, **kwargs):
    ...
    end_time = time.time()
    print_log(f"----time_consuming :{end_time - start_time} {task_name} status:{status} msg:{msg}")
    if status != 0:
        raise Exception(f"task {task_name} Execution_exception_status:{status} msg:{msg}")


def compile_resources(compile_source_res_dir: str, compiled_resources: str, aapt2: str):
    """
    Compile res contents 
    :param compile_source_res_dir: res_Folder_path 
    :param compiled_resources: Output_path 
    :param aapt2: aapt2
    :return:
    """
...
class Bundletool:
    ...
    def check_system(self, apk_path, out_aab_path):
        print_log(f"[Current_system]:{get_system()}")
        print_log(f"[Current_system_JAVA_version]#####:")
        _, msg = execute_cmd("java -version")
        print_log(f"[Enter_apk]:{apk_path}")
        if not os.path.exists(apk_path):
            return -1, f"The_entered_apk_does_not_exist:{apk_path}"
        print_log(f"[Output_aab]:{out_aab_path}")
        ...
        if not os.path.exists(self.keystore):
            return -2, f"The_entered_keystore_does_not_exist:{self.keystore}"
        ...
        if get_system() in [MACOS, Linux]:
            ...
            if status != 0:
                return -999, "Signature_error"
        else:
            print_log("Window_does_not_check_to_avoid_libraries_without_openssl")
        print_log(f"##################################################")
        print_log(f"[apktool]:{self.apktool}")
        print_log(f"[apktool_version_number]:#####")
    ...
    def build_module_zip(self, temp_dir: str, module_name: str, input_resources_dir: str, out_module_zip_path: str):
        ...
        if os.path.exists(input_res_dir):
            try:
                task(f"[{module_name}]-Compile_resources", compile_resources, input_res_dir, compiled_resources, self.aapt2)
            except Exception as e:
                print_log(f"[{module_name}]-Compile_resource_error {str(e)}")
                pass
        task(f"[{module_name}]-Associated_resources", link_resources, link_base_apk_path, input_manifest, self.android,
             self.min_sdk_version,
             self.target_sdk_version, self.version_code, self.version_name, self.aapt2, compiled_resources)
        task(f"[{module_name}]-Unzip_resources_apk", unzip_file, link_base_apk_path, unzip_link_apk_path)
        task(f"[{module_name}]-Mobile_AndroidManifest", mv, temp_android_manifest_path, target_android_manifest_path)
        if os.path.exists(input_assets):
            task(f"[{module_name}]-Copy_assets", copy, input_assets, target_assets_path)
        if os.path.exists(input_lib):
            task(f"[{module_name}]-Copy_lib", copy, input_lib, target_lib_path)
        if os.path.exists(input_unknown):
            task(f"[{module_name}]-Copy_unknown", copy, input_unknown, target_unknown_path)
        if os.path.exists(input_kotlin):
            task(f"[{module_name}]-Copy_kotlin", copy, input_kotlin, target_kotlin_path)
        if os.path.exists(input_meta_inf_path):
            task(f"[{module_name}]-Process_the_original_apk_signature_information", delete_sign, input_meta_inf_path)
        if os.path.exists(input_meta_inf_path):
            task(f"[{module_name}]-Copy_META-INF", copy, input_meta_inf_path, target_mata_inf_path)
        if os.path.exists(input_resources_dir):
            task(f"[{module_name}]-Copy_dex", copy_dex, input_resources_dir, target_dex_path)
        task(f"[{module_name}]-Compressed_zip", zip_file, unzip_link_apk_path, out_module_zip_path)
        return 0, "success"
    ...
    def run(self, apk_path, out_aab_path, pad_reg=""):
        ...
        try:
            task("Environment_&_parameter_verification", self.check_system, apk_path, out_aab_path)
            task("Unzip_input_apk ", decode_apk, apk_path, decode_apk_dir, self.apktool)
            task("Parse_apk_information", self.analysis_apk, decode_apk_dir)
            if self.is_pad():
                module_name = "pad_sy"
                pad_module_temp_dir = os.path.join(temp_dir, module_name)
                package = self.apk_package_name
                task("Build_a_pad_module", create_pad_module_dir, pad_module_temp_dir, module_name, package)
                task("Move_resources_to_the_pad_module", pad_mv_assets, decode_apk_dir, pad_module_temp_dir, self.pad_reg)
                self.bundle_modules[module_name] = pad_module_temp_dir

            for name, path in self.bundle_modules.items():
                task(f"[{name}]-Build_the_module_compression_package", self.build_module_zip, temp_dir, name, path,
            ...
            task("Build_config_json", create_bundle_config_json, bundle_config_json_path, self.do_not_compress)
            task("Build_aab", build_bundle, self.bundletool, modules, temp_aab_path, bundle_config_json_path)
            task("sign", sign, temp_aab_path, self.keystore, self.storepass, self.keypass, self.alias)
            task("Copy_output_copy", copy, temp_aab_path, out_aab_path)
        except Exception as e:
            print_log(e)
            return -1, str(e)
        finally:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="apk_to_aab")
    parser.add_argument("-i", "--input", help="Enter_the_path_of_the_apk", required=True)
    parser.add_argument("-o", "--output", help="Path_to_output_aab", required=True)
    parser.add_argument("--keystore", help="Signature_file_path", default=KEYSTORE)
    parser.add_argument("--store_password", help="Signature_file_path",
                        default=STORE_PASSWORD)
    parser.add_argument("--key_alias", help="Signature_file_path", default=KEY_ALIAS)
    parser.add_argument("--key_password", help="Signature_file_path", default=KEY_PASSWORD)
    parser.add_argument("--apktool", help="apktool.jar_path",
                        default=APKTOOL_PATH)
    parser.add_argument("--aapt2", help="aapt2_path", default=AAPT2_PATH)
    ...
    parser.add_argument(
        "--pad_reg", help="Extract the pad resource from the Assets directory, and match the file copy through regularization", default="")
